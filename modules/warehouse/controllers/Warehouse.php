...
if(strlen($data['inventory_filter']) > 0){
	...
	{
		$data['delivery_id'] = $id;
		$data['title'] = _l('مدیریت لیست بسته بندی');
	...
	public function get_serial_number()
	{
		if ($this->input->is_ajax_request()) {
			...
			foreach ($list_temporaty_serial_numbers as $index => $serial_number) {
				...
				$table_serial_number .= '<td class="serial_number">' . render_select('serial_number['.$index.']', $arr_serial_numbers,array('name','name'),'',$serial_number['serial_number'],[], ["data-none-selected-text" => _l('wh_serial_number')], 'no-margin', '', false) . '</td>';
