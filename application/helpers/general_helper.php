...
function _jd($date)
{
    ...
    $format = get_current_date_format();
//    $nformat = str_replace('%','' , $format);
//    $timestamp = '';
//    $d = DateTime::createFromFormat($nformat, $date);
//    if($d !== false) {
...
function _jdt($date, $is_timesheet = false, $newFormat = 'Y/m/d g:i A')
{
    $original = $date;
    ...
    return jdate($newFormat,$timestamp);

    //return hooks()->apply_filters('after_format_datetime', $date, ['original' => $original, 'is_timesheet' => $is_timesheet]);
}

function mz_to_sql_date($date)
{
    ...
    try {
        $dateTime = new DateTime($date);
        $formatted = $dateTime->format(str_replace('%', '', $format));
        $thisTimestamp = $dateTime->getTimestamp();
    } catch (Exception $e) {
    }
