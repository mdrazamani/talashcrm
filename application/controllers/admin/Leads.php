...
class Leads extends AdminController
{
    ...
    public function index($id = '')
    {
        ...
        if(!empty($id)){
            $data['mz_lead'] = $this->leads_model->mz_get_lead($id);
        }

        $this->load->view('admin/leads/manage_leads', $data);
    }

//    public function indexTrash($id = '')
//    {
//        close_setup_menu();
//
//        if (!is_staff_member()) {
...
//        // in case accesed the url leads/index/ directly with id - used in search
//        $data['leadid'] = $id;
//        $this->load->view('admin/leads/trashes', $data);
//    }
    ...
    public function new_lead(){
        ...
        $this->load->view('admin/leads/add_lead',$data);
    }

    public function add_too_lead(){
        $this->leads_model->add_too_lead($this->input->post()['addToolead_id'],$this->input->post()['lead_staff']);
        redirect(admin_url('leads'));
    }

    public function add_trash(){
        $this->leads_model->add_trash($this->input->post()['addTrash_id'],$this->input->post()['trash_staff']);
        redirect(admin_url('leads'));
    }

    public function trashes($id = ''){
        //close_setup_menu();
        ...
        $data['statuses'] = $this->leads_model->get_status();
        $data['sources']  = $this->leads_model->get_source();
        $data['title']    = _l('سطل مختومه ها');
        // in case accesed the url leads/index/ directly with id - used in search
        $data['leadid'] = $id;

        $data['trashes'] = $this->load->leads_model->get_all_trash();
        $rtash_i=0;
        foreach($data['trashes'] as $trash){
            //contact
            $contactIds = unserialize($trash['contactId']);
            array_push($data['t'],$contactIds);
            $str = '';
            foreach($this->load->leads_model->get_all_contacts($contactIds) as $lastname){
                $str.= ','.$lastname->title.' '.$lastname->lastname;
            }
            $data['trashes'][$rtash_i]['new_contacts'] = substr($str,1);
            //contact

            //added
            $data['trashes'][$rtash_i]['mzAdded'] = $this->load->leads_model->get_added($trash['addedfrom']);
            //added

            //assigned
            $data['trashes'][$rtash_i]['mzAssigned'] = $this->load->leads_model->get_assigned($trash['assigned']);
            //assigned
        ...
        $lead = $data['mz_lead'] = $this->leads_model->get($id, $leadWhere);

        //-----------------------------------------
        if(!empty($id)){
            $data['Litems'] = $this->leads_model->get_this_leads_items($id);
            $data['Lclient'] = $this->leads_model->get_this_leads_client($lead->customerId);
            $contactIds = unserialize($lead->contactId);
            $data['Lcontacts'] = $this->leads_model->get_this_leads_contacts($contactIds);
            $data['mainStaff'] = $this->leads_model->get_this_leads_staff($lead->addedfrom);
            $data['thisStaff'] = $this->leads_model->get_this_leads_this_staff($lead->assigned);
            $data['trashStaff'] = $this->leads_model->get_this_leads_this_staff($lead->trash_staff);
           // $data['trashStaff'] = 'hello';
        }
        //-----------------------------------------
            //$data['mz_lead'] = $this->leads_model->mz_get_lead(14);
        //}


        $this->load->view('admin/leads/manage_trash',$data);
    }

    public function convert_model($id =''){
        //$data['lead'] = $this->load->leads_model->mz_get_lead($id);
        $data['idoo'] = $id;
        $this->load->view('admin/leads/convertToProject',$data);
    }

    public function convert_modal($id =''){
...
//                die;
        ...
        ///////// l
        $data['lead'] = $this->leads_model->mz_get_lead($id);
        $data['title_i'] = '<span class="dash-micon"><i class="my-ti ti ti-adjustments-alt"></i></span>';
        $data['title'] = 'تبدیل به پروژه';
        $data['add_items'] = $this->leads_model->get_add_items($id);
        ///////// l
    ...
    public function nowConvertToProject(){
        //if ($this->input->post()) {
            $data = $this->input->post();
            $id = $this->leads_model->insert_into_projects($data);

            echo json_encode([
                'success' => $id
            ]);

        //$id = $this->projects_model->mz_add($data);
//                 echo json_encode([
//                    'data' => $data,
//                     //'id' => $id
//                ]);
...
//                }
                //$id = $this->projects_model->mz_add($data);
                //$id = $this->projects_model->convertToProject($data);
...
//        ]);
        //-----------------------------------------
        if(!empty($inputData['idForPdf'])){
            $id = $inputData['idForPdf'];
            $leadWhere = (has_permission('leads', '', 'view') ? [] : '(assigned = ' . get_staff_user_id() . ' OR addedfrom=' . get_staff_user_id() . ' OR is_public=1)');
            $lead = $data['mz_lead'] = $this->leads_model->get($id, $leadWhere);
            $data['Litems'] = $this->leads_model->get_this_leads_items($id);
            $data['Lclient'] = $this->leads_model->get_this_leads_client($lead->customerId);
        ...
        $html = $this->load->view('admin/pdfStyles/'.$title_key.'/'.substr($title_key,0,strlen($title_key)-1).'_H',$data,true);
        $css = $this->load->view('admin/pdfStyles/'.$title_key.'/'.substr($title_key,0,strlen($title_key)-1).'_C',$data,true);

        //  ok

        require_once 'Mypdfs.php';
        $mpdf = new Mypdfs();
        $mpdf->index($html, $css);
...
//       echo json_encode([
//           'test' => admin_url('application/views/admin/pdfStyles/leads/lead_H.php')
//       ]);

        //C:\wamp64\www\talash_crm\application\views\admin\pdfStyles\leads
        //http://localhost/talash_crm/application/views/admin/pdfStyles/leads/lead_H.php
...
//        echo "<br/>";
//        echo base_url();
           // echo file_get_contents('C:\wamp64\www\talash_crm\application\views\admin\pdfStyles\leads/lead_H.php');


        //$mpdf->index(file_get_contents('C:\wamp64\www\talash_crm\pdfStyles\leads\lead_H.php'),file_get_contents('C:\wamp64\www\talash_crm\pdfStyles\leads\lead_C.css'));

        //$html = $this->load->view('admin/leads/test');
        //$mpdf->index('<h1>سلام</h1>');

//        echo json_encode([
//            'data' => $mpdf->sey_hello()
//        ]);

        //header('Location: admin/logo/lgoo');
...
//            ->set_output(json_encode($outputData));
        ...
        //return json_encode(['a'=> 'mohammaderza']);
...
//        ];
        ...
        //$this->load->view('admin/leads/test',$data);

    }

    public function add_lead_new(){
        $data = $this->input->post();
        $this->leads_model->add($data);
        redirect(admin_url('leads'));
...
//        ]);
    ...
    private function _get_lead_data($id = '')
    {
        ...
        return [
            'data'          => $this->load->view('admin/leads/lead', $data, true).
                               $this->load->view('admin/pdfjs/pdf', $pdfData, true),
    ...
    public function trash_table(){
        $this->app->get_table_data('leads_trash');
    }
    ...
    public function add_note($rel_id)
    {
        ...
        if ($this->input->post()) {
            ...
            // Causing issues with duplicate ID or if my prefixed file for lead_H.php is used
