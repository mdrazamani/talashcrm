...
//    public function edit_peyment(){
//        $proposalsId = $_GET["proposals"];
//        $PeymentId = $_GET["editPeyment"];
//
//        redirect(admin_url('proposals?editPeyment=7#'.$proposalsId));
//    }
    //------mz
    public function addPishPeyment()
    {
        $data = $this->input->post();
        $this->proposals_model->add_Pish_Peyment($data);
         redirect(admin_url('proposals#' . $data['proposal']));
    }
    //------mz
    ...
    public function get_proposal_data_ajax($id, $to_return = false)
    {
        ...
        $data['pishPayments']          = $this->payments_model->get_pish_payments($id);

        //$number = new Number_word;
        //$data['allPriceHorof']         = numberToWords(2354);
        ...
        $data['staff_data'] = $this->staff_model->get_staff($data['proposal']->addedfrom);

        //--------------------------------------------------------------------------
        $contacts =  $this->proposals_model->get_contacts_data(unserialize($data['proposal']->project_data->notify_contacts));
        $data['contacts'] = $contacts;
        $customerContact='';
        foreach ($contacts as $contact) {
            $gender = $contact->sex == 'female' ? 'خانم ' : 'آقای ';
            $customerContact .= '- '.$contact->title.':'.' '.$gender.' '.$contact->lastname.',<br/>';
        }

        $gender = $data['staff_data'][0]->sex == 'female' ? "خانم " : "آقای ";
       $content = '
...
مدت اعتبار: 3روز,
<br/>
زمان تحویل: '.$data["proposal"]->send_time.' ,
<br/>
شیوه پرداخت: '.$data["proposal"]->peymentType.',
<br/>
- محل تحویل: انبار شرکت نگار صنعت ماهان
<br/>
- درخواست شماره: '.$data["proposal"]->project_data->id.'
<br/>
'.$customerContact.'
- کارشناس فروش: '.$gender.$data['staff_data'][0]->lastname.'
<br/>
- حداکثر زمان ارسال تایید فنی توسط کارفرما یک هفته پس از تحویل کالا بوده و عدم ارسال تاییدیه پس از آن به منزله تایید فنی کالاها محسوب
خواهد شد.
<br/>
- شماره شباي  IR240570021701101435967001بانک پاسارگاد بنام شرکت نگار صنعت ماهان
<br/>
- قیمتهاي فوق بر اساس نرخ فروش اسکناس یورو سامانه سنا در تاریخ صدور پیش فاکتور تنظیم و محاسبه گردیده است.لذا در صورت افزایش نرخ
ارز قیمتها تعدیل می گردد.
<br/>
            </td> 
        ...
        $data['proposal']->content = $content;
        $data['add_items'] = $data['proposal']->items;
        //--------------------------------------------------------------------------
    ...
    public function record_pishInvoice_payment_ajax($id)
    {
        ...
        $data['proposals']  = "hello";
        $data['payments'] = "hello";
        $this->load->view('admin/proposals/peyment_table', $data);
    }
