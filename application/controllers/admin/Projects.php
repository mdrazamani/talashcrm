            ...
            if ($id == '') {
                ...
                $id = $this->projects_model->add($data);
                 if ($id) {
                    ...
                    //redirect(admin_url('proposals/test.php' . $data));
                //$this->load->view('admin/proposals/test', $data);
...
//                die;
    ...
    public function view($id)
    {
        if (staff_can('view', 'projects') || $this->projects_model->is_member($id)) {
            ...
            $data['thisStaff'] = $this->mz_model->get_this_staff();
            if (!$data['tab']) {
                show_404();
            }
            ...
            $data['vendors'] = $this->projects_model->get_vendors();

            //exit($data['vendors']);

            $data['deparments'] = $this->projects_model->get_deparments();

            $data['project_items'] = $this->load->projects_model->get_project_items($data['project']->id);
            //$data['itemsWork'] = $this->load->projects_model->get_projects_workItems($data['project']->id);

            $dp = $this->load->projects_model->get_dp($_SESSION['staff_user_id']);

            $data['thisViews'] = $dp;
            $data['actionValues'] = $this->projects_model->showBazargani($_GET['initialID']);
            $data['ItemDetailsValues'] = $this->projects_model->item_details_id($_GET['itemId']);
            $data['itemAbleValues'] = $this->projects_model->get_mz_itemAble($data['ItemDetailsValues']->itemId);
            //------
            $data['currencies']    = $this->currencies_model->get();
    ...
    public function checkUpdateItem(){
        $inputs = $this->input->post();
        $result = $this->projects_model->item_details_MainNew($inputs['item_id'],$inputs['project_id']);
        $children = $this->projects_model->item_details_childNew($result->id);
        echo json_encode([
            'result' => array(
                'main' => $result,
                'child' => $children,
    ...
    public function getGivesLists($projectsID, $type){
        $this->app->get_table_data('list_mz_gets_project',['project_id' => $projectsID, 'type' => $type]);
    }

    public function getGivesLists_kharegi($projectsID, $type){
        $this->app->get_table_data('list_mz_gets_project_kharegi',['project_id' => $projectsID, 'type' => $type]);
    }


    public function pList($number,$type){
        $this->app->get_table_data('list_mz_project',['project_id' => $number, 'type' => $type]);
    }

    public function pChildList($number, $type, $id){
        $this->app->get_table_data('list_mz_project_child',['project_id' => $number, 'type' => $type, 'id' => $id]);
    }

    public function proposalLists($number,$type){
        $this->app->get_table_data('proposalLists',['project_id' => $number, 'type' => $type]);
    }
    public function getProposalLists($number,$type,$itemId){
        $this->app->get_table_data('getProposalLists',['project_id' => $number, 'type' => $type, 'itemIdController' => $itemId]);
    }
    public function gList($number){
        $this->app->get_table_data('list_mz_sends_project',['project_id' => $number]);
    }

    public function testList(){
        $data['test'] = unserialize('a:6:{i:0;a:7:{s:4:"cost";s:2:"12";s:12:"typeCurrency";s:1:"0";s:8:"typehaml";s:1:"0";s:8:"timehaml";s:2:"12";s:13:"timehamlVahed";s:1:"0";s:10:"timetahvil";s:2:"12";s:15:"timetahvilVahed";s:1:"0";}i:1;a:7:{s:4:"cost";s:2:"13";s:12:"typeCurrency";s:1:"0";s:8:"typehaml";s:1:"0";s:8:"timehaml";s:2:"13";s:13:"timehamlVahed";s:1:"0";s:10:"timetahvil";s:2:"13";s:15:"timetahvilVahed";s:1:"0";}i:2;a:7:{s:4:"cost";s:2:"14";s:12:"typeCurrency";s:1:"0";s:8:"typehaml";s:1:"0";s:8:"timehaml";s:2:"14";s:13:"timehamlVahed";s:1:"0";s:10:"timetahvil";s:2:"14";s:15:"timetahvilVahed";s:1:"0";}i:3;a:7:{s:4:"cost";s:2:"15";s:12:"typeCurrency";s:1:"0";s:8:"typehaml";s:1:"0";s:8:"timehaml";s:2:"15";s:13:"timehamlVahed";s:1:"0";s:10:"timetahvil";s:2:"15";s:15:"timetahvilVahed";s:1:"0";}i:4;a:7:{s:4:"cost";s:2:"16";s:12:"typeCurrency";s:1:"0";s:8:"typehaml";s:1:"0";s:8:"timehaml";s:2:"16";s:13:"timehamlVahed";s:1:"0";s:10:"timetahvil";s:2:"16";s:15:"timetahvilVahed";s:1:"0";}i:5;a:7:{s:4:"cost";s:2:"17";s:12:"typeCurrency";s:1:"0";s:8:"typehaml";s:1:"0";s:8:"timehaml";s:2:"17";s:13:"timehamlVahed";s:1:"0";s:10:"timetahvil";s:2:"17";s:15:"timetahvilVahed";s:1:"0";}}');
        $data['test2'] = 'test2';
        return $this->load->view('admin/projects/test_view', $data);
    }

    public function showJobDepartment(){
        $vId = $this->input->post();
        $data['contacts_vendor'] = $this->load->projects_model->get_job_position($vId['departmentId']);
        echo json_encode([
            'contacts' => $data['contacts_vendor']
        ]);
    }

    public function updateItemfn(){
        $data = $this->input->post();
        $update = $this->load->projects_model->updateItemfnModel($data);
        echo json_encode([
            'update' => $update
        ]);
    }
    ...
    public function updateItemfnUp(){
        $data = $this->input->post();
        $update = $this->load->projects_model->updateItemfnUpModel($data);
        echo json_encode([
            'update' => $update
        ]);
    }

    public function showContactsVendors(){
        $vId = $this->input->post();
        $data['contacts_vendor'] = $this->load->projects_model->get_vendor_contacts($vId['vendorId']);
        echo json_encode([
            'contacts' => $data['contacts_vendor']
        ]);
    }
    ...
    public function addSend(){
        $data = $this->input->post();
        $update = $this->load->projects_model->MzSendPm($data);
        echo json_encode([
            'result' => $update
        ]);
    }
    public function addSend2(){
        $data = $this->input->post();
        $update = $this->load->projects_model->MzSendPm2($data);
        echo json_encode([
            'result' => $update
        ]);
    }
    ...
    public function showPms(){
        $data = $this->input->post();
        $show = $this->projects_model->showPmsdb($data);
        echo json_encode([
            'resShow' => $show
        ]);
    }
    public function showPms2(){
        $data = $this->input->post();
        $show = $this->projects_model->showPmsdb2($data);
        echo json_encode([
            'resShow' => $show
        ]);
    }

    public function showStaffDepartment(){
        $vId = $this->input->post();
        $data['contacts_vendor'] = $this->load->projects_model->get_deparments_staff($vId['departmentId']);
        echo json_encode([
            'contacts2' => $data['contacts_vendor']
        ]);
    }

    public function sendItem(){
        $data = $this->input->post();
        $data['send']['sendDate'] = time();
    ...
    public function addAdakheliProcess(){
        $data = $this->input->post();
        $id = $this->load->projects_model->add_bazargani_lead($data);
        /*
        echo json_encode([
            'test' => $res
          //  'test' => $data
        ...
        */
        ...
        set_alert('success', _l('با موفقیت اضافه شد'));
        redirect(admin_url('projects/view/' . $data['projectID'] . '?group=project_thisViews_list'));
    }

    public function item_table()
    {

        //echo json_encode(['a'=> 'mohammaderza']);

//        $this->app->get_table_data('project_items',[
...
//            ]
    ...
    public function update_str_price_item(){
        $data = $this->input->post();
        $data['mainPrice']['date'] = time();
        $data['mainPrice']['addedfrom'] = $_SESSION['staff_user_id'];
        $qtyArray = $this->projects_model->get_item_sell_count($data['mainPrice']['itemId']);
        $data['mainPrice']['endPrice'] *= $qtyArray['qty'];
        $data['mainPrice']['qty'] = $qtyArray['qty'];
        $data['mainPrice']['unit'] = $qtyArray['unit'];
        $show = $this->projects_model->update_str_price_item_model(serialize($data['mainPrice']), $data['mainPrice']['itemId']);
        echo json_encode([
            'alert' => $show
        ]);
    }

    public function getAjaxHamlONaghl(){
        $input_data = $this->input->post();
        $main_data = $this->projects_model->getAjaxHamlONaghl_model($input_data);
        $main_data = unserialize($main_data);
        echo json_encode([
            'hamls' => $main_data
        ]);
    }
