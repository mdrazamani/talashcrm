...
class Proposals_model extends App_Model
{
    ...
    public function get_contacts_data($array){
        $contacts = [];
        foreach($array as $contact){
            array_push($contacts, $this->db->query("SELECT * FROM `tblcontacts` WHERE `id`='".$contact."'")->row());
        }
        return $contacts;
    }
    ...
    public function add_Pish_Peyment($data)
    {
        $this->db->insert(db_prefix() . 'invoicepaymentrecordspish', [
            'pishid'   => $data['proposal'],
            'amount'   => $data['amount'],
            'paymentmode'   => $data['paymentmode'],
            'date'   => $data['date'],
            'daterecorded'   => date('Y-m-d g:i:s'),
            'note'   => $data['note'],
