...
class Leads_model extends App_Model
{
    ...
    public function get_this_leads_items($id){
        $result = $this->db->query('SELECT * FROM `tblitemable` LEFT JOIN `tblcustomfieldsvalues` ON `tblcustomfieldsvalues`.`relid` = `tblitemable`.`id` WHERE `tblcustomfieldsvalues`.`fieldto` = "items" AND `rel_id` ="'.$id.'" AND `rel_type`="leads"')->result_array();
        if(!empty($result[0])){
            return $result;
        }else{
            return $this->db->query('SELECT * FROM `tblitemable` WHERE `rel_id` ="'.$id.'" AND `rel_type`="leads"')->result_array();
        }
    }

    public function get_this_leads_client($id){
        return $this->db->query('SELECT * FROM `tblclients` WHERE `userid` = "'.$id.'"')->row();
    }

    public function get_this_leads_staff($id){
        return $this->db->query('SELECT * FROM `tblstaff` WHERE `staffid`="'.$id.'"')->row();
    }
    public function get_this_leads_this_staff($id){
        return $this->db->query('SELECT sex,firstname,lastname FROM `tblstaff` WHERE `staffid`="'.$id.'"')->row();
    }

    public function get_this_leads_contacts($ids){
        $contacts = [];
        foreach ($ids as $id){
           array_push($contacts, $this->db->query('SELECT * FROM `tblcontacts` WHERE `id` = "'.$id.'"')->row());
        }
        return $contacts;
    }
    ...
    public function add($data)
    {
        ...
        $data['contactId'] = serialize($data['contactId']);


        $hook = hooks()->apply_filters('before_lead_added', [
            'data'  => $data,
            'items' => $items,
        ]);
        ...
        $data['addedfromName'] = $this->leads_model->get_assigned($data['addedfrom']);
        $data['assignedName'] = $this->leads_model->get_assigned($data['assigned']);
        ...
        if ($insert_id) {
            ...
            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'leads')) {
                    _maybe_insert_post_item_tax($itemid, $item, $insert_id, 'leads');
                }
            }
...
//        return $this->db->query('SELECT lastname FROM `tblstaff` WHERE `staffid` = "'.$id.'"')->row()->lastname;
//    }

    public function get_all_trash(){
        return $this->db->query('SELECT * FROM `tblleads` WHERE `is_trash` = "1"')->result_array();
    }

    public function get_all_leads(){
        return $this->db->query('SELECT * FROM `tblleads` WHERE `is_trash` = "0"')->result_array();
    }
    public function get_add_items($id){
        $items = [];
        $a_items = $this->db->query('SELECT * FROM `tblitemable` WHERE `rel_id` ="'.$id.'" and `rel_type`="leads"')->result_array();
        foreach ($a_items as $item){
           $b_items = $this->db->query('SELECT * FROM `tblcustomfieldsvalues` WHERE `relid` = "'.$item["id"].'" and `fieldto`="items"')->row();
            if(!empty($b_items->value)){
                $item['product_number'] = $b_items->value;
            }
    ...
    public function mz_get_lead($id){
        return $this->db->query('SELECT * FROM `tblleads` WHERE `id` = "'.$id.'"')->row();
    }

    public function get_assigned($id){
        return $this->db->query('SELECT lastname FROM `tblstaff` WHERE `staffid` = "'.$id.'"')->row()->lastname;
    }
    public function get_added($id){
        return $this->db->query('SELECT lastname FROM `tblstaff` WHERE `staffid` = "'.$id.'"')->row()->lastname;
    }
    public function add_trash($id,$staff){
        //return $this->db->query('update `` set `is_trash`="1" , `date_trash`="'.time().'" where `id`="'..'" ');

        $staff_trash_name = $this->leads_model->get_assigned($staff);
        $this->db->set('is_trash', '1');
        $this->db->set('trash_staff', $staff);
        $this->db->set('staff_trash_name', $staff_trash_name);
        $this->db->set('date_trash', time());
        $this->db->where('id', $id);
        $res = $this->db->update('tblleads');
        return $res;
    }


    public function add_too_lead($id,$staff){
        //return $this->db->query('update `` set `is_trash`="1" , `date_trash`="'.time().'" where `id`="'..'" ');

        $this->db->set('is_trash', '0');
        $this->db->set('addedfrom', $staff);
        $this->db->where('id', $id);
        $res = $this->db->update('tblleads');
        return $res;
    }

    public function get_all_contacts($contacts){
        $array = array();
        foreach ($contacts as $contact){
            array_push($array, $this->db->query('SELECT lastname,title FROM `tblcontacts` WHERE `id` = "'.$contact.'"')->row());
        }
        return $array;
    }
    ...
    public function insert_into_projects($data){
        ...
        $this->db->query("INSERT INTO `" . db_prefix() . "projects`(`name`, `description`, `status`, `clientid`, `billing_type`, `start_date`, `deadline`, `project_created`, `date_finished`, `progress`, `progress_from_tasks`, `project_cost`, `project_rate_per_hour`, `estimated_hours`, `addedfrom`, `contact_notification`, `notify_contacts`,`is_lead`) VALUES ('".$data['name']."','".$data['description']."','".$data['status']."','".$data['clientid']."','".$data['billing_type']."','".$data['start_date']."','".$data['deadline']."','".time()."','','".$data['progress']."','','".$data['project_cost']."','".$data['project_rate_per_hour']."','".$data['estimated_hours']."','".$_SESSION['staff_user_id']."','".$data['contact_notification']."','".$data['notify_contacts']."','".$data['is_lead']."')");
        $insert_id = $this->db->insert_id();

        if ($insert_id) {

            $this->db->where('id', $data['is_lead']);
            $this->db->update(db_prefix() . 'leads', [
                'is_project' => $insert_id
            ]);
