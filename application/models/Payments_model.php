...
class Payments_model extends App_Model
{
    ...
    //mz
    public function get_pish_payments($pishid)
    {
        $this->db->select('*,' . db_prefix() . 'invoicepaymentrecordspish.pid as paymentid');
        $this->db->join(db_prefix() . 'payment_modes', db_prefix() . 'payment_modes.id = ' . db_prefix() . 'invoicepaymentrecordspish.paymentmode', 'left');
        $this->db->order_by(db_prefix() . 'invoicepaymentrecordspish.pid', 'asc');
        $this->db->where('pishid', $pishid);
        $payments = $this->db->get(db_prefix() . 'invoicepaymentrecordspish')->result_array();
        // Since version 1.0.1
        $this->load->model('payment_modes_model');
        $payment_gateways = $this->payment_modes_model->get_payment_gateways(true);
        $i                = 0;
