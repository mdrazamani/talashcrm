...
array_push($where, 'AND is_trash = "0"');
...
foreach ($rResult as $aRow) {
    ...
    $row[] = ($aRow['lead_value'] != 0 ? number_format($aRow['lead_value']).' '.$base_currency->symbol : '');
    ...
    $assignedOutput = '';
    if ($aRow['addedfrom'] != 0) {
        $full_name = $aRow['addedfromName'];

        $assignedOutput = '<a data-toggle="tooltip" data-title="' . $full_name . '" href="' . admin_url('profile/' . $aRow['addedfrom']) . '">' . staff_profile_image($aRow['addedfrom'], [
    ...
    if ($aRow['assigned'] != 0) {
        ...
        $addedOutput = '<a data-toggle="tooltip" data-title="' . $full_name . '" href="' . admin_url('profile/' . $aRow['assigned']) . '">' . staff_profile_image($aRow['assigned'], [
                'staff-profile-image-small',
            ]) . '</a>';

        // For exporting
        $addedOutput .= '<span class="hide">' . $full_name . '</span>';
    }
...
//    $row[] = '<span data-toggle="tooltip" data-title="' . _dt($aRow['dateadded']) . '" class="text-has-action is-date">' . time_ago($aRow['dateadded']) . '</span>';
    ...
    $btn='';
    switch ($aRow['Dforce']){
        case '1': $btn='<span class="btn btn-danger">خیلی زیاد</span>'; break;
        case '2': $btn='<span class="btn btn-warning">زیاد</span>'; break;
        case '3': $btn='<span class="btn btn-info">متوسط</span>'; break;
        case '4': $btn='<span class="btn btn-primary">کم</span>'; break;
        case '5': $btn='<span class="btn btn-success">خیلی کم</span>'; break;
    }

    $row[] = $btn;
    $is_project = !empty($aRow["is_project"]) ? '<a href="'.admin_url("projects/view/".$aRow["is_project"]).'" class="btn btn-success">پروژه شده</a>' : '<span class="btn btn-warning">پروژه نشده</span>';
    $row[] = $is_project;
