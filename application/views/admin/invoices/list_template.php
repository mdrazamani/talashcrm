   ...
   <div class="mz-panel mbot10">
      <div class="mz-panel-body _buttons">
         ...
         <?php if(has_permission('invoices','','create')){ ?>
            <a href="<?php echo admin_url('invoices/invoice'); ?>" class="btn btn-info pull-left new new-invoice-list mright5"><?php echo _l('افزودن فاکتور جدید'); ?></a>
         <?php } ?>
         <?php if(!isset($project)){ ?>
               <a href="<?php echo admin_url('invoices/recurring'); ?>" class="btn btn-info pull-left mright5">
                  <?php echo _l('فاکتور های تکراری'); ?>
               </a>
         <?php } ?>
          <?php if (!isset($project) && !isset($customer) && staff_can('create', 'payments')) { ?>
              <button id="add-batch-payment" onclick="add_batch_payment()" class="btn btn-info pull-left">
                  <?php echo _l('پرداخت های دسته ای'); ?>
              </button>
          <?php } ?>
