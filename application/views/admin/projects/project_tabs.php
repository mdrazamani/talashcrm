...
<div class="horizontal-scrollable-tabs">
  ...
  <div class="horizontal-tabs">
    <ul class="nav nav-tabs no-margin project-tabs nav-tabs-horizontal display-flex" role="tablist">
        <?php
        foreach(filter_project_visible_tabs($tabs, $project->settings->available_features) as $key => $tab){
            ...
            <li class="<?php if($key == 'project_overview' && !$this->input->get('group')){echo 'active ';} ?>project_tab_<?php echo $key; ?><?php if($dropdown){echo ' nav-tabs-submenu-parent';} ?>">
                ...
                <?php if($dropdown){ ?>
                    ...
                    <?php
                        if($tab['name'] == 'tabNameNullSee'){
                            echo $thisStaff->position_name;
                        }else{
                            echo $tab['name'];
                        }
                    ?>
                    ...
                    <?php if(!is_rtl()){ ?>
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdown_<?php echo $key; ?>">
                            ...
                            foreach($tab['children'] as $d){
                                ...
                                if($d['slug'] == 'project_thisViews_systems_gives'){
                                    if($thisViews['dp']['en_name'] != 'forosh' && $thisViews['dp']['en_name'] != 'fani'){
                                        continue;
                                    }
                                }
                                if($d['slug'] == 'project_thisViews_proposal'){
                                    if($thisViews['dp']['en_name'] != 'forosh'){
                                        continue;
                                    }
                                }

                                if($d['slug'] == 'project_thisViews_lead'){
                                    continue;
                                }
