...
<div class="mz-panel">
    <div class="mz-panel-header">
        <span class="dash-mtext"> لیست </span>
    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="send[staff]" id="staff" style="background-color: #fff;" class=" form-control btn-default" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                            <option value="0">ابتدا موقعیت شغلی را انتخاب کنید!</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group select-placeholder">
...
<!-- -->
<div class="modal fade" id="allPMoadl" style="overflow-y: scroll; background-color: rgba(0,0,0,0.6);" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" onclick="showAllP('0', '0', 'end')" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalAllPLabel">
                    <?php echo _l('شماره '); ?>
                </h4>
            </div>
            <div class="modal-body">
...
//                    _l(''),
                ...
                render_datatable($table_data,'pChildList');
                ?>
                <!-- -->
                <?php
                hooks()->add_action('app_admin_footer', function(){
                ...
                <script>
                    ...
                    //     });
                    //     //console.log(trash_ServerParams);
                    //     initDataTable('.table-pChildList', admin_url+'projects/pChildList/'+$('#projectId').val()+'/'+$('#type').val(), ['undefined'], ['undefined'], trash_ServerParams, [0, 'desc']);
                    // })
                </script>
    ...
    $(document).ready(function () {
                ...
                success:function(data){
                    ...
                    if(result.contacts.length !== 0){
                        ...
                        for(let i=0; i<result.contacts.length; i++){
                            ...
                            select_contacts.innerHTML += `<option value="23">23</option>`;
       ...
       //let select_contacts2 = document.getElementById('staff');
        ...
        sendBtn.addEventListener('click',()=>{
            $.ajax({
                url: '<?php echo site_url("admin/projects/addSend");?>',
                type: 'post',
                data:{
                    //allSend: send
                    department:department_P.value,
...
<script>
    ...
    function showPm(proposalId){
        ...
        resultSendShow.innerHTML = ``;
        $.ajax({
            url: '<?php echo site_url("admin/projects/showPms");?>',
            type: 'post',
            data:{
...
<script>
    function showAllP(key, id, type){
        let allPMoadl = document.getElementById('allPMoadl');
        let myModalAllPLabel = document.getElementById('myModalAllPLabel');
        if(type === 'start'){
            allPMoadl.classList.add('in');
            allPMoadl.style.display="block";
            myModalAllPLabel.innerHTML = 'شماره ';
            myModalAllPLabel.innerHTML += key;
            ...
            $(function(){
                ...
                //console.log(trash_ServerParams);
                initDataTable('.table-pChildList', admin_url+'projects/pChildList/'+$('#projectId').val()+'/'+$('#type').val()+'/'+id, ['undefined'], ['undefined'], trash_ServerParams, [0, 'desc']);
            })



        }else{
            //allPMoadl.classList.remove('in');
            //allPMoadl.style.display="none";
            location.href='';
        }
