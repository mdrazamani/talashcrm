   ...
   <div class="col-md-6 border-right project-overview-left">
      ...
      <div class="col-md-7">
         <table class="table no-margin project-overview-table">
            <tbody>
              <tr class="project-overview-id">
                  <td class="bold"><?php echo _l('پروژه'); ?> <?php echo _l('the_number_sign'); ?></td>
                  <td>
                      <?php echo $project->id; ?>
                  </td>
              </tr>
              <tr class="project-overview-customer">
                  <td class="bold"><?php echo _l('مشتری'); ?></td>
                  <td>
               ...
               <tr class="project-overview-billing">
                  <td class="bold"><?php echo _l('نوع صورتحساب'); ?></td>
                  <td>
                     <?php
                     if($project->billing_type == 1){
                       $type_name = 'نرخ ثابت';
                    } else if($project->billing_type == 2){
                       $type_name = 'ساعت پروژه';
                    } else {
                       $type_name = 'ساعت کار';
                    }
                    echo _l($type_name);
                 ...
                 <?php if($project->billing_type == 1 || $project->billing_type == 2){
                  ...
                  if($project->billing_type == 1){
                    echo '<td class="bold">'._l('کل مبلغ برآورد شده').'</td>';
                    echo '<td>'.app_format_money($project->project_cost, $currency).'</td>';
                 } else {
                    echo '<td class="bold">'._l('نرخ پروژه هر یک ساعت').'</td>';
         ...
         <tr class="project-overview-estimated-hours">
            <td class="bold<?php if(hours_to_seconds_format($project->estimated_hours) < (int)$project_total_logged_time){echo ' text-warning';} ?>"><?php echo _l('کل زمان برآورد شده(ساعت)'); ?></td>
            <td><?php echo str_replace('.', ':', $project->estimated_hours); ?></td>
         </tr>
         <?php } ?>
         <?php if(has_permission('projects','','create')){ ?>
         <tr class="project-overview-total-logged-hours">
            <td class="bold"><?php echo _l('مجموع ساعات ثبت شده'); ?></td>
...
<div class="col-md-5 text-center project-percent-col mtop10">
   <p class="bold"><?php echo _l('میزان پیشرفت پروژه'); ?></p>
...
<div class="tags-read-only-custom project-overview-tags">
   ...
   <?php echo '<p class="font-size-14"><b><i class="fa fa-tag" aria-hidden="true"></i> ' . _l('برچسب ها') . ':</b></p>'; ?>
...
<div class="tc-content project-overview-description">
   <hr class="hr-panel-heading project-area-separation" />
   <p class="bold font-size-14 project-info"><?php echo _l('شرح پروژه'); ?></p>
...
<div class="team-members project-overview-team-members">
   <hr class="hr-panel-heading project-area-separation" />
   <?php if(has_permission('projects','','edit')){ ?>
   <div class="inline-block pull-right mright10 project-member-settings" data-toggle="tooltip" data-title="<?php echo _l('افزودن/ویرایش اعضا'); ?>">
   ...
   <div class="media">
      ...
      <div class="media-body">
         ...
         <h5 class="media-heading mtop5"><a href="<?php echo admin_url('profile/'.$member["staff_id"]); ?>"><?php echo get_staff_full_name($member['staff_id']); ?></a>
            ...
            <br /><small class="text-muted"><?php echo _l('مجموع ساعات ثبت شده') .': '.seconds_to_time_format($member['total_logged_time']); ?></small>
            <?php } ?>
         </h5>
      </div>
   </div>
